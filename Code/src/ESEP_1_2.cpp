...
int main(int argc, char** args) {
    ...
    thread encoderT = thread(&encoderThread, &channelID_Encoder, &channelID_Dispatcher);
    thread dispatcherT = thread(&dispatcherThread, &channelID_Dispatcher);

    /* Waiting for demo */
    if (DEMO) {
        printf("Demo time for %d minutes...\n", DEMO_DURATION);
        this_thread::sleep_for(chrono::minutes(DEMO_DURATION));
        printf("Stopping in...\n");
        for (int i = 1; i < 4 ; ++i) {
            printf("%d\n",i);
