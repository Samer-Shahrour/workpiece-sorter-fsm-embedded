...
void operationMode(_pulse msg, int connectionID_Dispatcher) {
    ...
    static std::vector<int> averageVector;

    if(hight < flatMin - 1000){
        MsgSendPulse(connectionID_Dispatcher, -1, PULSE_HIGHT, ERROR_HIGHT); // ERRORHIGHT
        return;
    }
    ...
    if(measuring_wp && (hight < beltMin)){
        averageVector.push_back(hight);
        if(hight < maxHight){
            maxHight = hight;
        }

    }
    if(measuring_wp && (beltMax > hight) && (hight > beltMin+50)){
        ...
        for(int i = 0; i < averageVector.size(); i++){
            average += averageVector[i];
        }
        average = average / averageVector.size();
        printf("ENCODER: AVERAGE HIGHT: %d mm\n", (((beltMax + beltMin) / 2) - average) / pointsPerMM);
...
void updatePoints(){
    int belt = (beltMax + beltMin) / 2;
    int flat = (flatMin + flatMax) / 2;
    pointsPerMM = (belt - flat)/FLAT_IN_MM;
}
