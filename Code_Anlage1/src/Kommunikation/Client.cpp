...
#define TWO_MACHINES false

void client(_pulse msg){
    if(TWO_MACHINES) {
        int conid;
        if ((conid = name_open(ATTACH_POINT_M1, NAME_FLAG_ATTACH_GLOBAL)) <
            0) {   // important!! if server is running with "NAME_FLAG_ATTACH_GLOBAL" flag, you hae to use it here too
